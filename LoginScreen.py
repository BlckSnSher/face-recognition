...
from logic import checkInputLogin
...
class LoginScreen:
    ...
    def login(self):
        user_name = self.entry_1.get()
        password = self.entry_2.get()
        c = checkInputLogin(user_name, password)
        if c:
            conn = con.connect(
                host="localhost",
                user="root",
            ...
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM users WHERE username = '{user_name}' AND password = '{password}'")
            ...
            messagebox.showerror(
                "Error", "Provide a correct username and password to log in")
